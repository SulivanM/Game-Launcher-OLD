...
<div class="container">
  ...
  <main>
    ...
    <script>
        console.log('Avant la création de Pusher');
const pusher = new Pusher('{{ config('broadcasting.connections.pusher.key') }}', {
    cluster: '{{ config('broadcasting.connections.pusher.options.cluster') }}',
    encrypted: true,
});
console.log('Après la création de Pusher');

        // Écoute des événements
const channel = pusher.subscribe('friend-requests');

channel.bind('FriendRequestSent', (event) => {
    // Ajoutez le code pour afficher la notification de demande d'ami en temps réel
...
channel.bind('FriendRequestAccepted', (event) => {
    // Ajoutez le code pour afficher la notification d'acceptation de la demande d'ami en temps réel
    document.getElementById('real-time-notifications').innerHTML +=
        `<div>${event.user.name} accepted your friend request</div>`;
});

channel.bind('FriendRequestDeclined', (event) => {
    // Ajoutez le code pour afficher la notification de refus de la demande d'ami en temps réel
    document.getElementById('real-time-notifications').innerHTML +=
